...
public interface CommentRepository extends PagingAndSortingRepository<CommentEntity, Long> {
    ...
    @Query(value = "" +
            ...
            "     ),\n" +
            "     same_branch as (\n" +
            "         select resac.channel_id,\n" +
            "                vids.video_id,\n" +
            "                max(res.AUTHOR_TEXT_ID) as AUTHOR_TEXT_ID\n" +
            "         from comments com,\n" +
            "              comment_ids comids,\n" +
            "              author_channels comac,\n" +
            "              comments rep,\n" +
            ...
            "           and repac.channel_id = ?1\n" +
            "           and repac.channel_id != comac.channel_id\n" +
            "           and res.comment_id = resid.id\n" +
            "           and comids.thread_id = resid.thread_id\n" +
            "           and res.author_channel_id != com.author_channel_id\n" +
            "           and res.author_channel_id != rep.author_channel_id\n" +
            "           and res.author_channel_id = resac.id\n" +
            "           and res.video_id = vids.id\n" +
            "         group by resac.channel_id, vids.video_id),\n" +
            "     all_union as (\n" +
            "         select *, 1 rp, 0 rr, 0 st, 0 sb\n" +
            "         from posters\n" +
            "         union\n" +
            "         select *, 0 rp, 1 rr, 0 st, 0 sb\n" +
            "         from recipients\n" +
            "         union\n" +
            "         select *, 0 rp, 0 rr, 1 st, 0 sb\n" +
            "         from same_thread\n" +
            "         union\n" +
            "         select *, 0 rp, 0 rr, 0 st, 1 sb\n" +
            "         from same_branch),\n" +
            "     agg_no_author_texts as (\n" +
            "         select CHANNEL_ID                                             authorchannelid,\n" +
            "                max(AUTHOR_TEXT_ID)                                    AUTHOR_TEXT_ID,\n" +
            "                array_agg(distinct case when rp = 1 then VIDEO_ID end) reppostervideos,\n" +
            "                array_agg(distinct case when rr = 1 then VIDEO_ID end) reprecipientvideos,\n" +
            "                array_agg(distinct case when st = 1 then VIDEO_ID end) samethreadpostervideos,\n" +
            "                array_agg(distinct case when sb = 1 then VIDEO_ID end) samebranchpostervideos\n" +
            "         from all_union au\n" +
            "         group by CHANNEL_ID\n" +
            "         order by bit_or(rp) + bit_or(rr) + bit_or(st) + bit_or(sb) desc, sum(rp + rr + st + sb) desc\n" +
            "         limit 10\n" +
            "     )\n" +
